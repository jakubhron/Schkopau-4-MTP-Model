...
def build_model(df: pd.DataFrame, cost_meta: dict) -> ConcreteModel:
    ...
    m._has_duo = _has_duo
    if _has_duo:
        _duo_cost_adj: dict = {}   # pre-computed (Δslope × Pnom + Δfixed) per (b,t)
        for b in cfg.BLOCKS:
            ...
            pmax_eff_b = cost_meta[f"Pmax_eff_{b}"]
            if duo_slope_col in idx.columns:
                ...
                for t in range(T_len):
                    _duo_cost_adj[(b, t)] = 0.0

        m.duo_cost_adj = Param(m.B, m.T, initialize=_duo_cost_adj)
    ...
    m.run_costs = Var(m.B, m.T, bounds=(None, None))
    ...
    # Also fix tier variables to 0 → single flat startup cost (warm),
    # eliminating ~36K binaries and all tier classification constraints.
    if cfg.USE_SIMPLE_STARTUP_RAMP:
        for b in cfg.BLOCKS:
            for t in range(T_len):
                m.in_ramp[b, t].fix(0)
                m.hot_start[b, t].fix(0)
                m.cold_start[b, t].fix(0)
                m.vcold_start[b, t].fix(0)
    ...
    if cfg.USE_COAL_CONSTRAINS and coal_limits:
        ...
        # DUO coal adjustment — linearised at Pnom like cost adjustment
        if _has_duo:
            _duo_coal_adj: dict = {}
            for b in cfg.BLOCKS:
                ...
                if duo_col in idx.columns:
                    ...
                    for t in range(T_len):
                        _duo_coal_adj[(b, t)] = 0.0
            m.duo_coal_adj = Param(m.B, m.T, initialize=_duo_coal_adj)
...
def _add_power_bounds(m) -> None:
    ...
    # Tight per-(b,t) Big-M for Pmin relaxation during ramp
    # Need M >= Pmin + boost so constraint is non-binding when in_ramp=1
    _M_ramp = {}
    for b in m.B:
        for t in m.T:
            _M_ramp[(b, t)] = float(value(m.Pmin[b, t])) + _boost + 1.0

    def p_lower(m, b, t):
        # Pmin relaxed during startup ramp hours (in_ramp=1)
        return m.P[b, t] >= m.Pmin[b, t] * m.on[b, t] + _boost * m.both_on[t] - _M_ramp[(b, t)] * m.in_ramp[b, t]
    ...
    for b in m.B:
        ...
        for t in m.T:
            ...
            for h in range(_H):
                ...
                # Tight Big-M: UB needs M >= Pmax+boost - ramp_val;
                #               LB needs M >= ramp_floor (so P>=0 is not violated)

                # --- Upper bounds (each tier tightens if active) ---
                if h < len(hot_ramp):
                    _m_ub = pmax_bt + _boost - hot_ramp[h] + 1.0
                    cl_ramp.add(
                        m.P[b, t] <= hot_ramp[h] + _boost * m.both_on[t]
                        + _m_ub * (1 - m.hot_start[b, s])
                    )
                if h < len(warm_ramp):
                    _m_ub = pmax_bt + _boost - warm_ramp[h] + 1.0
                    cl_ramp.add(
                        m.P[b, t] <= warm_ramp[h] + _boost * m.both_on[t]
                        + _m_ub * (1 - m.startup[b, s])
                    )
                if h < len(cold_ramp):
                    _m_ub = pmax_bt + _boost - cold_ramp[h] + 1.0
                    cl_ramp.add(
                        m.P[b, t] <= cold_ramp[h] + _boost * m.both_on[t]
                        + _m_ub * (1 - m.cold_start[b, s])
                    )
                if h < len(vcold_ramp):
                    _m_ub = pmax_bt + _boost - vcold_ramp[h] + 1.0
                    cl_ramp.add(
                        m.P[b, t] <= vcold_ramp[h] + _boost * m.both_on[t]
                        + _m_ub * (1 - m.vcold_start[b, s])
                    )
